#!/bin/bash
# This script deploys the local terraform infrastructure
# It is assumed that the project structure is as imported
#------------------------------------------------------------
echo "This script deploys the local terraform infrastructure \n"
...
terraform init  -backend-config="region=$AWS_REGION" -backend-config="bucket=$TF_STATES_BASE_BUCKET_NAME" -backend-config="key=$AWS_TF_STATE_KEY"
terraform apply
...
echo "Deploying Snowflake infrastructure \n"
cd "../snowflake"
...
terraform fmt --recursive
terraform init -backend-config="region=$AWS_REGION" -backend-config="bucket=$TF_STATES_BASE_BUCKET_NAME" -backend-config="key=$SNOWFLAKE_TF_STATE_KEY"
terraform apply